...
async def upload_to_s3(file_path: str) -> str:
    ...
    from app.services.queue_service import upload_to_s3_task
    import os
    
    # Get S3 configuration
    bucket = os.getenv("AWS_BUCKET_NAME", "flowai-videos")
    key = f"exports/{file_path}"
    
    # Queue S3 upload task
    task = upload_to_s3_task.delay(file_path, bucket, key)
    
    # For now, return CDN URL (task will complete in background)
    # In production, you might want to wait for task completion
